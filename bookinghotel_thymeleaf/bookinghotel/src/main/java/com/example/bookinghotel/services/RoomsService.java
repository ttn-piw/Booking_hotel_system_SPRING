...
import com.example.bookinghotel.exception.RoomNotFoundException;
...
public class RoomsService {
    ...
    public List<room> getRoomsByHID(Integer hotelID) {
        ...
        List<room> rooms = entityManager.createQuery(getRooms).getResultList();
        if (rooms.isEmpty())
            throw new RoomNotFoundException("Hotel id not found"+hotelID);
    ...
    @Transactional
    public List<room> getRoomsByHID_api(Integer hotelID) {
//
        String query = "SELECT r FROM room r WHERE r.hotels.HID = :hotel_id";
        List<room> rooms = entityManager.createQuery(query, room.class)
